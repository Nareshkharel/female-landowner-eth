...
outreg2 using summary_stats_1.doc, replace sum(log) keep(chemical_fertilizer improved_maize crop_rotation age basic_educ male_head dependency_ratio non_farm_enterprise  wealth_index dist_admhq female_landowner dist_road  soil_fertility drought_shock sfi joint_ownership sole_female_ownership sole_male_ownership)
...
tobit fies_score sfi male_head age basic_educ non_farm_enterprise dependency_ratio wealth_index dist_admhq sole_female_ownership joint_ownership drought_shock soil_fertility i.region, vce(robust)

summarize fies_score


/////// regression part   //////////////////

///// agricultural practices regression
...
/// frequency plot of fies category by region

tab saq01
gen region = "Afar" if saq01== 2
replace region = "Amhara" if saq01== 3
replace region = "Oromia" if saq01== 4
replace region = "Somali" if saq01== 5
replace region = "Benishangul" if saq01== 6
replace region = "SNNPR" if saq01== 7
replace region = "Gambela" if saq01== 12
replace region = "Harar" if saq01== 13
replace region = "Dire Dawa" if saq01== 15

bysort region: tab fies_category


/*   no bar chart inlcuded

graph bar (cum_percent), over(fies_category) over(region) asyvars stack ///
title("Percentage Distribution of Food Insecurity by Region") ///
ytitle("Percentage") legend(order(1 "Low" 2 "Moderate" 3 "Severe"))


graph bar (percent), over(fies_category) over(region, gap(20) label(angle(90))) asyvars stack ///
title("Food Insecurity by Region") ///
ytitle("Percentage") legend(order(1 "Food secure" 2 "Moderate FI" 3 "Severe FI"))



drop count - percent_region

bysort saq01: tab fies_category

drop cum_percent

* Calculate frequencies
bys saq01 fies_category: gen count = _N  

* Get total count per region
bys saq01: egen total_region = total(count)  

* Calculate percentage within each region
gen percent = (count / total_region) * 100  

* Sort data for cumulative calculation
sort saq01 fies_category  

* Calculate cumulative percentage
bys saq01: gen cum_percent = sum(percent)
graph bar (percent), over(fies_category) over(saq01) asyvars stack ///
title(" Food Insecurity by Region") ///
ytitle("Percentage") ///
ylabel(0(20)100) ///
legend(order(1 "Low" 2 "Moderate" 3 "Severe"))
...
gen malehh_jointowner = male_head * joint_ownership


logit fies_dummy sfi age age_sq basic_educ male_head dependency_ratio   wealth_index female_landowner soil_fertility drought_shock married livestock_hh i.saq01, vce(robust)
margins, dydx (*) post

outreg2 using "/Users/nareshkharel/Desktop/Thesis/ETH_2021_ESPS-W5_v01_M_Stata_1/codes/spec_paper.doc", replace ctitle(" FI") dec(3)

logit fies_dummy sfi age age_sq basic_educ male_head dependency_ratio wealth_index sole_female_ownership joint_ownership  soil_fertility drought_shock married livestock_hh i.saq01, vce(robust)
margins, dydx (*) post

outreg2 using "/Users/nareshkharel/Desktop/Thesis/ETH_2021_ESPS-W5_v01_M_Stata_1/codes/spec_paper.doc", append ctitle(" FI") dec(3)


logit severe_fi sfi age age_sq basic_educ male_head dependency_ratio wealth_index female_landowner soil_fertility drought_shock married livestock_hh i.saq01, vce(robust)
margins, dydx (*) post

outreg2 using "/Users/nareshkharel/Desktop/Thesis/ETH_2021_ESPS-W5_v01_M_Stata_1/codes/spec_paper.doc", append ctitle("Sev FI") dec(3)


logit severe_fi  sfi age age_sq basic_educ male_head dependency_ratio  wealth_index sole_female_ownership joint_ownership  soil_fertility drought_shock married livestock_hh i.saq01, vce(robust)
margins, dydx (*) post

outreg2 using "/Users/nareshkharel/Desktop/Thesis/ETH_2021_ESPS-W5_v01_M_Stata_1/codes/spec_paper.doc", append ctitle("Sev FI") dec(3)


logit severe_fi sfi age age_sq basic_educ male_head dependency_ratio non_farm_enterprise  wealth_index sole_female_ownership joint_ownership soil_fertility shock_faced married total_land livestock_hh dist_admhq dist_road i.saq01, vce(robust)

logit fies_dummy sfi age age_sq basic_educ male_head dependency_ratio non_farm_enterprise  wealth_index sole_female_ownership joint_ownership soil_fertility shock_faced married total_land livestock_hh dist_admhq dist_road i.saq01, vce(robust)

logit severe_fi sfi age age_sq basic_educ male_head dependency_ratio non_farm_enterprise  wealth_index sole_female_ownership joint_ownership soil_fertility shock_faced married total_land livestock_hh dist_admhq dist_road i.saq01, vce(robust)

logit severe_fi sfi age age_sq basic_educ male_head dependency_ratio non_farm_enterprise  wealth_index sole_female_ownership joint_ownership soil_fertility shock_faced married total_land livestock_hh psnp_labour i.saq01, vce(robust)


logit fies_dummy age age_sq basic_educ male_head dependency_ratio non_farm_enterprise  wealth_index female_landowner soil_fertility shock_faced married total_land livestock_hh i.saq01, vce(robust)
margins, dydx (*) post
