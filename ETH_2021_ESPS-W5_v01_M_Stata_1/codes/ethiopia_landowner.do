...
//// sole female landowner



/////// joint female landowner and sole female landowner ///////

* Step 1: Generate a temporary variable to track male and female ownership for each household
gen male_owner = 0
gen female_owner = 0


* Step 2: Loop through each land ownership variable
foreach var in s2q04b_1 s2q04b_2 s2q04b_3 s2q04b_4 {
...
* Step 3: Create the joint land ownership variable
...
////// documented ownership : right to sell land 
gen indfemale_landowner2 = 0   
foreach var in  s2q07_1 s2q07_2   {
    replace indfemale_landowner2 = 1 if `var' == individual_id  & s1q02  == 2
}

bysort household_id: egen female_landowner2 =max(indfemale_landowner2)
tab female_landowner2

label variable female_landowner2 "1= at least one female in HH has right to sell one plot of land, 0 = no right to sell"

gen indmale_landowner2 = 0   
foreach var in  s2q07_1 s2q07_2   {
    replace indmale_landowner2 = 1 if `var' == individual_id  & s1q02  == 1
}

bysort household_id: egen male_landowner2 =max(indmale_landowner2)

gen joint_ownership_sell = (female_landowner2 == 1 & male_landowner2==1)

gen sole_femaleown_sell =  (female_landowner2 == 1 & male_landowner2==0)
gen sole_maleown_sell = (female_landowner2 == 0 & male_landowner2==1)


drop if missing(s2q07_1 )

//// sole female has right to sell
...
bysort household_id: egen legume_crop = min(s3q34)
replace legume_crop = 0 if legume_crop== 2
...
label variable legume_crop "used legume crop in last 3 years, 1= Yes, 0=No"
